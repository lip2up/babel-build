...
class Build {
    ...
    build(jpath, depMap = {}) {
        ...
        this.doBuild(jpath, result)
        result.codeList.push(`define.require('${jpath}')`)
        return result
    }

    doBuild(jpath, { codeList, depList, depMap }) {
        depMap[jpath] = 1
        depList.push(jpath)

        const source = fs.readFileSync(path.join(this.baseDir, jpath), { encoding: 'utf8' })
        ...
        const { code, deps } = this.cache.cached(hash, () => {
            ...
            const { code } = parser(source, jpath, ext == 'js' ? this.depDump.getBabelPlugin({ hash }) : null)
